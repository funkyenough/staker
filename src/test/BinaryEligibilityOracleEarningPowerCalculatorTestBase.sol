...
abstract contract BinaryEligibilityOracleEarningPowerCalculatorTestBase is StakerTestBase {
  ...
  {
    console2.log("calling binaryEligibilityCalculator overridden _stake");
    ...
    console2.log("depositId in _stake: ", Staker.DepositIdentifier.unwrap(_depositId));
    // This should only be triggered on the first call
    // Subsequent calls will have the delegateeScore already above threshold,
    // so their stakes should be full already.
    ...
    console2.log("return _depositId in _stake: ", Staker.DepositIdentifier.unwrap(_depositId));
    return _depositId;
  }
}
